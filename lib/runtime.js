...
module.exports = {
	...
	// obj[mm.PrepareInvoke](args: array, env, context) : boolean
	// Prepares invocation of a smart function.
	// Return true to enable invocation.
	// Return false if some of the arguments are blocked.
	// Elements of the args array may be rewritten by the method.
	PrepareInvoke: '@metamaya-Invoke',


	// obj[mm.Invoke](...args)
	// Invokes a smart function.
	Invoke: '@metamaya-Invoke',
	...
	enclose(expr, env) {
		if (!mm.isReducible(expr))
			return expr;
		else if (expr instanceof mm.model.Closure)
			return expr;
		else
			return new mm.model.Closure(expr, env);
	}
}
