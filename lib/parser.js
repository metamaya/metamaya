...
function translateOperator(optype, op, args) {
	...
	return new mm.prog.Invocation(null, func, args);
}
...
const modelActions = {
	...
	File(stms) {
		let root = new mm.prog.Constructor(mm.prog.Prototype, stms.model());
	...
	Definition_namedObject(id, lbrace, stms, rbrace, t) {
		return new mm.prog.Definition(id.model(), new mm.prog.Constructor(mm.prog.Prototype, stms.model()));
	},
	//Definition_namedObjectExtend(id, colon, expr, lbrace, stms, rbrace, t) {
	//	return new mm.prog.Definition(id.model(), new mm.prog.Constructor(expr.model(), stms.model()));
	//},
	Definition_namedFunction(id, params, eq, expr, t) {
		let func = new mm.prog.Function(params.model(), expr.model());
	...
	Primary_getProperty(obj, op, id) {
		return new mm.prog.PropertyReference(obj.model(), id.model());
	},
	Primary_index(obj, lbracket, key, rbracket) {
		return new mm.prog.PropertyReference(obj.model(), key.model());
	},
	Primary_invoke(func, args) {
		return new mm.prog.Invocation(new mm.prog.This(), func.model(), args.model());
	},
	Primary_invokeProperty(obj, pt, id, args) {
		return new mm.prog.Invocation(obj.model(), new mm.prog.KeyReference(id.model(), false), args.model());
	},
	Primary_parens(lp, expr, rp) {
		return expr.model();
	},
	Primary_object(lbrace, stms, rbrace) {
		return new mm.prog.Constructor(mm.prog.Prototype, stms.model());
	},
	Primary_objectExtend(expr, lbrace, stms, rbrace) {
		return new mm.prog.Constructor(expr.model(), stms.model());
	},
	Primary_nameReference(id) {
		return new mm.prog.KeyReference(id.model(), true);
	},
	...
	Parameter(id) {
		return new mm.prog.Parameter(id.model());
	},
