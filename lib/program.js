...
function Module(body, env = undefined) {
	this.body = body;
...
Module.prototype = {
	[mm.Reduce](env, realm) {
		if (this.env !== env) {
			// specialize object
			return new mm.prog.Module(this.body, env);
		}

		if (!mm.rewriteProperty(this, 'body', global, realm))
			return this;
...
function PropertyReference(target, key, env) {
	this.target = target;
	this.key = key;
	this.env = env;
}
PropertyReference.prototype = {
	[mm.Reduce](env, realm) {
		if (this.env !== env) {
			// specialize the object to the current environment
			// That allows for rewriting this node without losing
			// referential transparency.
			return new mm.prog.PropertyReference(this.target, this.key, env);
		}
