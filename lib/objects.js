...
exports.Application = function Application(head, args, options) {
	this.head = head;
	this.args = args;
	if (options) {
		// options are meant to be non-enumerable
		if (options.rewritable)
			Object.defineProperty(this, 'rewritable', { value: true });
	}
}
exports.Application.prototype = {
	[mm.Normalize](ec) {
		if (!this.rewritable) {
			// make a rewritable copy
			return new mm.Application(this.head, this.args.slice(), { rewritable: true });
		}
